...
from .serializers import MevaSerializer

@api_view(['GET'])
def mevalar(request):
    mevalar = Meva.objects.all()
    serializer = MevaSerializer(mevalar,many=True)
    return Response(
        {
...
@api_view(['POST'])
def meva_qoshish(request):
    serializer = MevaSerializer(data=request.data)
    if serializer.is_valid():
        serializer.save()
        return Response(
            {
                "status": status.HTTP_201_CREATED,
                'message': "Meva qoshildi",
